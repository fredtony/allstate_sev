...
convert_cat_to_num(tr_te)
sparse_data.append(csr_matrix(minmax_scale(tr_te[f_prob_ordered])))

## Continuous data (1st label min/max values that are conspicuous)
for i in [1,3,5,6,7,9,10,11,12,13]:
    col = 'cont' + str(i)
    min_val = (tr_te[col]==tr_te[col].min())
    sparse_data.append(csr_matrix(min_val.astype('int')).transpose())
    tr_te.loc[tr_te[col]==min_val,col] = tr_te[col].median()
col = 'cont3'
max_val = (tr_te[col]==tr_te[col].max())
sparse_data.append(csr_matrix(max_val.astype('int')).transpose())
tr_te.loc[tr_te[col]==max_val, col] = tr_te[col].median()
