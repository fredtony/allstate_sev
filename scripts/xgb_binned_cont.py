...
#    print(train_test[categorical_feats])
    ...
    kf = kfold_regression(train_y, n_folds)
    for i, (train_index, cv_index) in enumerate(kf):
        ...
        cv_preds[cv_index] = np.exp(clf.predict(d_cv, ntree_limit=clf.best_ntree_limit))-shift
        cv_score = mean_absolute_error(np.exp(y_cv)-shift, cv_preds[cv_index])
        print('eval-MAE: %.6f' % cv_score)
        y_preds[:,i] = np.exp(clf.predict(d_test, ntree_limit=clf.best_ntree_limit)) - shift
