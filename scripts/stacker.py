# -*- coding: utf-8 -*-
"""
Created on Thu Oct 27 23:02:47 2016
...
def out_from_preds(train_ids, cv_pred, test_ids, test_pred, folds, score, clf_type):
    ...
    pred_file = '../L1/L1ensemble_submission_{}fold-{}-{:.4f}-{}.csv.gz'.format(
    ...
    oof_pred_file = '../L1/L1ensemble_oof_preds_{}fold-{}-{:.4f}-{}.csv.gz'.format(
        folds, clf_type, score, now.strftime("%Y-%m-%d-%H-%M"))
...
eval_slsqp(train, y, test, train_ids, test_ids, 10, 20)
...
#clf = KNeighborsRegressor(n_neighbors=10, weights='uniform', n_jobs=-1, p=1)
...
eval_clf(clf, train, y, test, train_ids, test_ids, 'GBR', 10, 2199)
...
def eval_xgb(train, y, test, train_ids, test_ids, nfolds=5, random_state=0):
    ...
    for i, (train_idx, cv_idx) in enumerate(skf):
        ...
        bst = xgb.train(xgb_params, dtrain, num_round, watchlist, obj=fair_obj,
            feval=xg_eval_mae, verbose_eval=50, early_stopping_rounds=early_stopping_rounds)
...
eval_xgb(train, y, test, train_ids, test_ids, nfolds=10, random_state=8)

def nn_model(n):
    model = Sequential()
    
    model.add(Dense(200, input_dim = n, init = 'he_normal', activation='relu'))
...
def eval_keras(train_cv, y, test, train_ids, test_ids, nfolds=10, random_state=0, nepochs=50):
    skf = kfold_regression(y, nfolds, random_state=random_state)
    test_pred = np.zeros((test.shape[0], nfolds))
    cv_pred = np.zeros(y.shape)
    scores = np.zeros(nfolds)
    for i, (train_idx, cv_idx) in enumerate(skf):
        ...
        model.fit(train, y_train, batch_size=1024, shuffle=True, verbose=0,
                        nb_epoch = nepochs, validation_data=(cv,y_cv))
        cv_pred[cv_idx] = model.predict(cv, batch_size=1000)
        scores[i] = mean_absolute_error(y_cv, cv_pred[cv_idx])
        print("Fold {} MAE: {}".format(i+1, scores[i]))
        test_pred[:,i] = model.predict(test, batch_size=1000).reshape((test_pred.shape[0],))
    del train, cv
    ...
    out_from_preds(train_ids, cv_pred, test_ids, test_pred, nfolds, score, 'keras')
eval_keras(train, y, test, train_ids, test_ids, 10, 1, 55)